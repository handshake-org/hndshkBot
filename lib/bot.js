...
bot.addListener('join', function (event) {
  ...
  if (who !== bot.user.nick) {
    bot.notice(who, 'Welcome ' + who + " to the Project Handshake Channel. Have you claimed a share of HNS yet? You can do so in two easy steps: First, generate your HNS wallet using this tool https://github.com/handshake-org/faucet-tool Second, submit the address directly to me using the :postaddress command. Note, your account must be over six months old. For accounts created on handshake.org, follow the instructions there and use the :code command.")
...
function sayHelp (to) {
  ...
  bot.say(to, '1. Download this tool to generate a wallet address -> https://github.com/handshake-org/faucet-tool')
  bot.say(to, '2. Submit your wallet address using the :postaddress command.')
  bot.say(to, '3. Done. Your coins will be waiting for you to claim them on the mainnet blockchain.')


  bot.say(to, 'commands ------------------------------------------------------------------------------------ ')
  bot.say(to, ':check       See if your freenode account qualifies for the faucet.')
  bot.say(to, ':postaddress       [wallet address] [full name] Submit address to be granted coins by faucet. Please include your full name for KYC purposes.')
...
async function registerUser (from, text) {
  let args = text.split(' ')
  let email = args[1]
  let full_name = args.slice(2, args.length).join(' ')
  ...
  let response = await handshake.registerUser({user: user, email: email, full_name: full_name})
  if (response.success) {
...
async function postAddress (from, text) {
  try {
    ...
    if (full_name.length < 2) {
      bot.say(from, 'You must provide a full name after your address. See :help')
      return false
    }
    if (address.slice(0, 2) !== 'hs' || address.length !== 42) {
      bot.say(from, 'You must provide a valid Handshake mainnet address as the first arg. See :help')
      return false
    }
    let user = await checkUser(from)
    if (user.qualified) {
      bot.say(from, 'Thank you. Saving that away')
      let response = await handshake.redeemUser({email: user.account, user: user, full_name: full_name, address: address})
      if (response.success) {
        bot.say(from, 'Address saved successfully for ' + response.account + ' under the name  ' + full_name)
      } else {
        bot.say(from, response.error)
        return false
      }
