...
function handlePublicMessage(event) {
  ...
  let from = event.nick;
  if(text.match(/:help/)) {
    bot.say(from, `Need help, ${from}? If you would like to redeeem your validation code from the Handshake Website, send me a PRVMSG in the format 'code: YOUR_CODE' and I will attempt to validate it for you. If you do not yet have an account on https://handshake.org go and create one first to get a validation code. `);
  } else if(text.match(/:tokens?/)) {
    checkUser(from)
...
function checkUserAndCode(from, text) {
  ...
  if(commands[0] === 'code:') {
    ...
    .then(user => {
      valid = verifyUser(user)
      if (valid) {
        bot.say(from, "Your account is Valid! Checking Code Now")
