...
function sayHelp (to) {
  ...
  bot.say(to, '3. Send me your wallet address using the :postaddress command.')

  bot.say(to, 'commands ------------------------------------------------------------------------------------ ')
  bot.say(to, ':register [email address] [full name]          Required for KYC/AML purposes. ')
  bot.say(to, ':check                                         See if your freenode account qualifies for the faucet.')
  bot.say(to, ':postaddress [email address] [wallet address]  Lookup the wallet address saved for your freenode account.')
...
async function postAddress (from, text) {
  bot.say(from, 'Ooh thank you. Saving that away')
  let args = text.split(' ')
  let email = args[1]
  let address = args[2]
  if (!/@/.test(email)) {
  ...
  if (user.qualified) {
    ...
    if (response.success) {
      ...
      bot.say(from, 'Something is not right. Have you already registered or saved an address? Check with :getaddress')
      return false
    }
  } else {
    bot.say(from, 'Sorry, your account is not old enough to qualify :(')
