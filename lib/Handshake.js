...
class Handshake {
  ...
  async redeemUser (params) {
    try {
      let registrant = await this.registerUser(params)
      if (!registrant.code) { throw new Error(registrant.error) }
      ...
      if (!freenode.code) {
        return { success: false, error: 'Please register before submitting an address.' }
      }
      ...
      if (!redeemed.success) {
        return { success: false, error: 'Something went wrong. Please try again or contact support@handshke.org' }
      }
